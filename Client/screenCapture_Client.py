...
def screenCapture(self, client): 
	...
	#Tạo canvas chứa ảnh đã chụp
		self.canvas = Canvas(self.Screenshot, bg = "white", width = 1070, height = 510) 	# Tạo canvas mới   
		self.canvas.place(relx = 0,rely = 0)   		# Vẽ ảnh chụp màn hình lên canvas
	#Tạo button Chụp ảnh	
		self.btnChup= PhotoImage(file='./img/button/button-capture.png')                      # Đặt hình ảnh
	...
	#Tạo button Lưu ảnh
		self.btnSave= PhotoImage(file='./img/button/button-save-image.png')                      # Đặt hình ảnh
	...
	#Tạo button Không lưu ảnh
		self.btnDSave= PhotoImage(file='./img/button/button-cancel.png')                      # Đặt hình ảnh
